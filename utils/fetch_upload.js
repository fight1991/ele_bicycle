...
// 方法统一包装
const ajaxFunc = async ({url, data, filePath, name, headerToken, isLoading, other, loadingText, func}) => {
  try {
    if (isLoading) showLoading(loadingText)
    let res = await func(url, data, filePath, name, headerToken)
...
wx.$upload = ({url, data, filePath, name, headerToken, isLoading = true, other = true, loadingText = '上传中...'}) => {
